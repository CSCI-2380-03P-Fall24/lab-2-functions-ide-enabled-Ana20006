...
int locateSmallest(int array[], int start, int end, int size) { 
    if (start < 0 || end >= size || end < start) { // Ensure valid range
        return -1; 
    }

    int minIndex = start;  // Start with the first element in range
    for (int i = start + 1; i <= end; ++i) {
        if (array[i] < array[minIndex]) {
            minIndex = i;  // Update if a smaller value is found
        }
    }
    return minIndex; // Return index of the smallest value
}
...
void replaceVariable(int array[], int target, int start, int end, int size) {
    if (start < 0 || end >= size || end < start) { // Ensure valid range
        return;
    }
...
int sumOdds(int array[], int start, int end, int size) {
    if (start < 0 || end >= size || end < start) { // Ensure valid range
        return -1; 
    }
